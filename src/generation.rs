...
impl GenNetwork {
        ...
        for layer in 1..self.node_array.len() {
            for node in 0..self.node_array[layer].len() {
                for prev_node in 0..self.node_array[layer - 1].len() {
                    ...
                    self.node_array[layer][node].link_vals[prev_node] =
                        Some(self.node_array[layer - 1][prev_node].cached_output.unwrap());
                    // I think this line needs to be un-commented
                    self.node_array[layer][node].output(&self.activation_function);
                    if DEBUG {
                        if layer == self.answer.unwrap() {
                            println!(
                                "Ran output on answer {:?}",
                                self.node_array[layer][node].cached_output
                            )
                        }
                    }
                }
    ...
    fn largest_node(&self) -> usize {
        ...
        (0..self.node_array[self.answer.unwrap()].len())
            .into_iter()
            .for_each(|node| {
                if self.node_array[self.answer.unwrap()][node].cached_output
    ...
    fn self_analysis<'b>(
        ...
        if !(epochs.is_none()) {
            // This lets us use the same function for testing and training
